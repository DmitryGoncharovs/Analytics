Сделаь n-day retention(анализ возвращаемости пользователей) приложения (1/3/7/14/30/60/90)дня
Разбить пользовтаелей по когортам
(В качестве признака когорты использовать месяц регистрации пользователя)
Построить наглядный дашборд
